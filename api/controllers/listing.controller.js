...
export const updateListing = async (req, res, next) => {
  ...
  if (req.user._id !== listing.userRef.toString()) {
    return next(
      errorHandler("You are not authorized to update this listing", 403)
    );
  }
...
export const getListings = async (req, res, next) => {
  try {
    ...
    let sortOption = { [sort]: order };

    // If sorting by price, use regularPrice field
    if (sort === "regularPrice") {
      sortOption = { regularPrice: order };
    }
    ...
    const listings = await Listing.find({
      ...
      .sort(sortOption) // Use the corrected sort option
      .limit(limit)
      .skip(startIndex);
